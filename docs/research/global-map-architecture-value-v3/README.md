...
| file | purpose |
|---|---|
| `execution-contract.md` | The backend-agnostic inference configuration contract: per-role fixed configuration, model-identity contract, fresh-context/startup-context contract, opaque generator condition-identity, packet-delivery/tool contract, raw-output immutability, provenance categories, true pre-unblind Git freeze, backend-qualification canary requirement, backend-agnostic empirical invariant. |
...
1. Checkout `3cc4975...`, read reused source-manifest + ledger (unchanged from V1/V2).
2. Select and preregister the execution backend for this run; run and record the **backend qualification canary** (`execution-contract.md` §Canary) before any experimental packet is built. If qualification fails, STOP — no synthetic fallback, no parent-agent-prose fallback, no template/heuristic fallback.
3. For each probe P01–P05 (P05 = Book4 paired with P03, per V2), build 3 condition packets per `../global-map-architecture-value-v2/condition-specification.md` §A/B/C substantive content, using **opaque, non-condition-correlated observation IDs** for all filenames/paths/delegation strings (`execution-contract.md` §Opaque generator condition identity).
4. Generate 3 independent observations per condition per probe under the fixed per-role inference configuration (`execution-contract.md` §Inference configuration contract), preserving every raw response immutably before any normalization (§Raw-output immutability).
...
6. Perform blinded evaluation under the evaluator's fixed inference configuration, again preserving raw responses immutably before normalization.
7. Freeze blinded judgments; create the **true pre-unblind Git commit** (`execution-contract.md` §Pre-unblind freeze) containing blind packets, raw outputs, raw evaluator responses, normalized judgments, hashes, schedule, and a hash/commitment of the sealed map — but not the readable mapping.
8. Only after that commit exists, reveal the mapping and mechanically join. Record post-unblind reconciliation in a later commit.
