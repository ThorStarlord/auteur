...
> **AGENT-NATIVE EMPIRICAL EXECUTION WITH DISCLOSED FROZEN-V2 EXECUTION DEVIATIONS — DIRECTIONALLY INTERPRETABLE FOR WITHIN-RUN A/B/C COMPARISON — NOT A STRICT TEMPERATURE/TOOLS-PINNED V2 REPLICATION.**
...
| **E5** | Generator packet filenames were condition-correlated (see §C). | Agent-native execution-contract deviation / leakage risk, not a frozen-V2 invalidation. Low practical materiality (no condition label or hidden signal in the packet *body*), but disclosed rather than assumed away. |

None of E1–E5 is, by itself or in combination, treated here as invalidating the A vs B vs C comparison outright, because: (a) each applies uniformly across conditions rather than differentially advantaging any one condition, and (b) the observed pattern (A weakest, with a specific, mechanistically explainable failure mode at P01; B and C both stronger; C strongest, concentrated at the Book-4 probes) is a plausible, mechanistically grounded signal rather than an artifact obviously attributable to any of E1–E5. But uniformity is not treated as automatic proof of irrelevance either — a reader should weigh E1–E5 when deciding how much confidence to place in the magnitude (not just the direction) of the observed gaps.
