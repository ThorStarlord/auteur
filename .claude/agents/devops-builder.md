---
name: devops-builder
description: Implements CI/CD and IaC changes (GitHub Actions, Terraform, Helm, Docker). Scoped to infra paths only. Skips if the brief has no infra changes.
...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit|NotebookEdit"
      hooks:
        - type: command
          command: 'node "$CLAUDE_PROJECT_DIR/.claude/hooks/factory-guard.mjs" devops-builder'
---
...
Authoritative scope: `CLAUDE.md` → "Path scoping for agents" → "DevOps Builder may edit". Common entries:
