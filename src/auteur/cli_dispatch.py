...
from auteur.cli_parser import _pilot_artifact_type, _pilot_project_root
...
from auteur.cli_handlers import (
    ...
    handle_identity_validate, handle_init, handle_publish, handle_retry,
...
def _write_blueprint_markdown(bp: Any, path: Path) -> None:
    ...
    lines.append("\n---")
    lines.append("*Generated by Auteur v0.26.0 — Blueprint Publish*")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def _write_structure_markdown(outlines: list[Path], project_root: Path, path: Path) -> None:
    """Render chapter outlines as a readable Markdown document."""
    lines: list[str] = []
    lines.append("# Chapter Structure")
    lines.append(f"Project: {project_root}")
    lines.append(f"Chapters: {len(outlines)}")
    lines.append("")

    import yaml as _yaml
    for o in outlines:
        try:
            data = _yaml.safe_load(o.read_text(encoding="utf-8"))
        except Exception:
            lines.append(f"## {o.parent.name} (parse failed)")
            lines.append("")
            continue
        if not data:
            continue
        ch_name = o.parent.name
        ch_index = data.get("chapter_index", data.get("chapter", ch_name))
        summary = data.get("chapter_summary", data.get("summary", ""))
        lines.append(f"## Chapter {ch_index}: {ch_name}")
        if summary:
            lines.append("")
        ...
        if scenes:
            ...
            for s in scenes:
                sid = s.get("scene_id", "?")
                pov = s.get("pov_character", s.get("pov", ""))
                loc = s.get("location", "")
                s_summary = s.get("summary", "")
                tension = s.get("estimated_tension", "")
                ...
                meta = " | ".join(parts)
                lines.append(f"- **{sid}**{': ' + meta if meta else ''}")
                if s_summary:
                    lines.append(f"  {s_summary}")
        arc = data.get("arc_pushes", [])
        if arc:
            lines.append("")
            lines.append(f"Arc pushes: {', '.join(arc)}")
        theme = data.get("thematic_reinforcement", "")
        if theme:
            lines.append(f"Thematic reinforcement: {theme}")
        lines.append("")

    lines.append("---")
    lines.append(f"*Generated by Auteur v{__import__('auteur').__version__} — Structure Publish*")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")

def _write_scene_markdown(scenes: list[Path], project_root: Path, path: Path) -> None:
    """Render scene artifacts as a readable Markdown document."""
    lines: list[str] = []
    lines.append("# Scene Realization Artifacts")
    lines.append(f"Project: {project_root}")
    lines.append(f"Scenes: {len(scenes)}")
    lines.append("")
    ...
    chapters: dict[str, list[tuple[str, dict]]] = {}
    for s in scenes:
        if s.name == "index.yaml":
            continue
        try:
            data = _yaml.safe_load(s.read_text(encoding="utf-8"))
        except Exception:
            lines.append(f"## {s.parent.name}/{s.name} (parse failed)")
            lines.append("")
            continue
        if not data:
            continue
        ch = data.get("chapter_id", data.get("chapter", s.parent.name))
        sid = data.get("id", s.stem)
        chapters.setdefault(ch, []).append((sid, data))

    # Keep scenes within each chapter sorted
    for ch in chapters:
        chapters[ch].sort(key=lambda pair: pair[0])

    for ch in sorted(chapters.keys()):
        scene_list = chapters[ch]
        lines.append(f"## Chapter: {ch} ({len(scene_list)} scenes)")
        lines.append("")
        for sid, data in scene_list:
            status = data.get("status", "draft")
            status_marker = {"draft": "[D]", "incomplete": "[I]", "ready": "[R]"}.get(status, f"[{status}]")
            pov = data.get("pov_character_id", data.get("pov_character", ""))
            participants = data.get("participants", [])
            narrative_pos = data.get("narrative_position", "")
            ...
            if narrative_pos:
                parts.append(f"Pos: {narrative_pos}")
            if story_time:
                parts.append(f"Time: {story_time}")
            meta = " | ".join(parts)
            lines.append(f"### {sid}")
            lines.append(f"**{meta}**")
            lines.append("")
            ...
            if goal:
                g_actor = goal.get("actor_id", "")
                g_obj = goal.get("objective", "")
                g_rationale = goal.get("rationale", "")
                lines.append(f"- **Goal**: {g_actor} — {g_obj}")
                if g_rationale:
                    lines.append(f"  *Rationale*: {g_rationale}")

            if opposition:
                o_source = opposition.get("source_id", "")
                o_pressure = opposition.get("pressure", "")
                o_rationale = opposition.get("rationale", "")
                lines.append(f"- **Opposition**: {o_source} — {o_pressure}")
                if o_rationale:
                    lines.append(f"  *Rationale*: {o_rationale}")

            if turn:
                t_type = turn.get("type", "")
                t_event = turn.get("event", "")
                t_impact = turn.get("impact", "")
                lines.append(f"- **Turn** ({t_type}): {t_event}")
                if t_impact:
                    lines.append(f"  *Impact*: {t_impact}")

            if outcome:
                o_result = outcome.get("result", "")
                o_knowledge = outcome.get("knowledge_added", [])
                o_consequences = outcome.get("consequences", [])
                lines.append(f"- **Outcome**: {o_result}")
                if o_knowledge:
                    lines.append(f"  *Knowledge added*: {'; '.join(o_knowledge)}")
                if o_consequences:
                    lines.append(f"  *Consequences*: {'; '.join(o_consequences)}")

            if tags:
                lines.append(f"- **Tags**: {', '.join(tags)}")

            realizes = data.get("realizes_arc_beats", [])
            if realizes:
                beat_lines = []
                for beat in realizes:
                    bid = beat.get("beat_id", "")
                    degree = beat.get("degree", "")
                    if degree:
                        beat_lines.append(f"{bid} ({degree})")
                    else:
                        beat_lines.append(bid)
                lines.append(f"- **Arc Beats**: {', '.join(beat_lines)}")

            setups = data.get("setups_created", [])
            if setups:
                lines.append(f"- **Setups**: {', '.join(setups)}")

            payoffs = data.get("payoffs_triggered", [])
            if payoffs:
                lines.append(f"- **Payoffs**: {', '.join(payoffs)}")
            ...
            exit_s = data.get("exit_state")
            if entry or exit_s:
                lines.append("")
                if entry:
                    ek = entry.get("knowledge", [])
                    ee = entry.get("emotional", {})
                    entry_detail = []
                    if ek:
                        entry_detail.append(f"{len(ek)} knowledge fact(s)")
                    if ee:
                        entry_detail.append(f"{len(ee)} emotion(s)")
                    lines.append(f"  Entry: {'; '.join(entry_detail)}")
                if exit_s:
                    xk = exit_s.get("knowledge", [])
                    xe = exit_s.get("emotional", {})
                    exit_detail = []
                    if xk:
                        exit_detail.append(f"{len(xk)} knowledge fact(s)")
                    if xe:
                        exit_detail.append(f"{len(xe)} emotion(s)")
                    lines.append(f"  Exit: {'; '.join(exit_detail)}")

            lines.append("")
        lines.append("")

    lines.append("---")
    lines.append(f"*Generated by Auteur v{__import__('auteur').__version__} — Scene Publish*")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def dispatch(args: argparse.Namespace) -> int:
    ...
    if args.command == "dashboard":
        ...
        try:
            ...
            _err(f"failed to build dashboard: {exc}")
            return 1
        if args.json:
            import json as _json
    ...
    if args.command == "audit":
        ...
        try:
            ...
            if ap is None:
                _err("no data to serialize")
                return 1
        except OSError as exc:
            _err(f"failed to write audit report to {dd / 'audit_report.json'}: {exc}")
            return 1
        d.artifact_path = ap
        result.data = d
        out = format_audit(result)
    ...
    if args.command == "structure" and args.structure_command == "diagnose":
        try:
            ...
            _err(f"invalid blueprint {args.blueprint}: {exc}")
            return 1
        ...
        if args.output:
            ...
            bpp = args.blueprint
            if bpp.is_dir():
                dd = Project.load(bpp).structure_diagnostics_dir()
            elif bpp.name == "blueprint.yaml" and (bpp.parent / "bible.json").exists():
                dd = Project.load(bpp.parent).structure_diagnostics_dir()
            else:
                dd = bpp.parent / "structure" / "diagnostics"
                dd.mkdir(parents=True, exist_ok=True)
            ap = dd / "structure_report.json"
        try:
            if serialize_structure_diagnose(result, ap) is None:
                _err("no data to serialize")
                return 1
        except OSError as exc:
            _err(f"failed to write report to {ap}: {exc}")
            return 1
    ...
    if args.command == "structure" and args.structure_command == "propose-repairs":
        try:
            bp, dd, pd = _bp_dirs(args.blueprint)
        except FileNotFoundError:
            _err(f"blueprint not found: {args.blueprint}")
            return 1
        except Exception as exc:
        ...
        try:
            ...
            _err(f"failed to write structure artifacts: {exc}")
            return 1
    ...
    if args.command == "structure" and args.structure_command == "apply":
        if not args.proposal.exists():
            _err(f"proposal not found: {args.proposal}")
            return 1
        ...
        if args.in_place and args.output is not None:
            ...
            return 1
        try:
            prop = StructureProposal.model_validate(
                yaml.safe_load(args.proposal.read_text(encoding="utf-8")))
        except (ValueError, yaml.YAMLError, OSError) as exc:
            _err(f"invalid proposal {args.proposal}: {exc}")
            return 1
        if prop.source_domain == "bible_audit":
            ...
            return 1
        try:
            bp, _, _ = _bp_dirs(args.blueprint)
        except Exception as exc:
            _err(f"invalid blueprint {args.blueprint}: {exc}")
            return 1
        src = args.blueprint / "blueprint.yaml" if args.blueprint.is_dir() else args.blueprint
        if prop.decision is not None:
            result = apply_impact_proposal(prop, bp,
                output_dir=str(args.output or src.parent) if not args.in_place else None,
                original_path=str(src) if args.in_place else None)
        else:
            result = handle_structure_apply(prop, bp,
                output_dir=str(args.output or src.parent) if not args.in_place else None,
                original_path=str(src) if args.in_place else None)
        if not result.is_success:
            _err(result.error)
            return result.exit_code
        # Refresh provenance hashes when the proposal is applied in-place
        if args.in_place:
            project_root = str(args.blueprint) if args.blueprint.is_dir() else str(args.blueprint.parent)
            try:
                freshness_updates = propagate_acceptance(prop, str(src), project_root)
                result.data["freshness_updates"] = freshness_updates
            except Exception as exc:
                # Non-blocking: warn but don't fail the apply
                print(f"Warning: failed to refresh provenance hashes: {exc}", file=sys.stderr)
        result.data["proposal_path"] = str(args.proposal)
        result.data["source_blueprint_path"] = str(src)
        if prop.decision is not None:
            out = format_apply_impact_proposal(result)
        else:
    ...
    if args.command == "structure" and args.structure_command == "generate":
        try:
            ...
            _err(f"blueprint file not found: {args.blueprint}")
            return 1
        except Exception as e:
            _err(f"failed to parse blueprint {args.blueprint}: {e}")
            return 1
        ...
        if d.get("is_diagnostics") and "diagnoses" in d:
            d["blueprint"] = str(args.blueprint)
            result.data = d
            out = format_structure_generate(result)
            if out:
                print(out)
            if args.output:
                serialize_structure_generate_text(out, args.output)
            if args.output:
        ...
        return 0
    # === structure publish ===
    if args.command == "structure" and args.structure_command == "publish":
        project_path = args.project
        output = args.output
        # Find chapter outline files
        chapters_dir = project_path / "chapters"
        outlines = sorted(chapters_dir.glob("*/outline.yaml")) if chapters_dir.is_dir() else []
        if not outlines:
            # Try .auteur location
            outlines = sorted(project_path.glob(".auteur/state/artifacts/chapters/*/outline.yaml"))
        if not outlines:
            _err("No chapter outlines found. Compile outlines first with 'auteur cartographer compile'.")
            return 1
        if output is None:
            output = project_path / "published" / "chapter-structure"
            output.mkdir(parents=True, exist_ok=True)
            output = output / f"structure.{args.format}"
        elif output.is_dir():
            output = output / f"structure.{args.format}"
        ...
        try:
            if args.format == "yaml":
                ...
                for o in outlines:
                    try:
                        combined.append(_yaml.safe_load(o.read_text(encoding="utf-8")))
                    except Exception:
                        combined.append({"file": str(o), "error": "parse failed"})
                _yaml.safe_dump(combined, output.open("w", encoding="utf-8"),
                                sort_keys=False, allow_unicode=True)
            else:
                _write_structure_markdown(outlines, project_path, output)
        except Exception as exc:
            _err(f"failed to publish structure: {exc}")
            return 1
    ...
    if args.command == "scene" and args.scene_command == "publish":
        ...
        if scenes_dir.is_dir():
            for genre_dir in sorted(scenes_dir.iterdir()):
                if genre_dir.is_dir():
                    scenes.extend(sorted(genre_dir.rglob("*.yaml")))
        if not scenes:
            _err("No scene files found. Seed scenes first with 'auteur realization seed'.")
            return 1
        if output is None:
            output = project_path / "published" / "scenes"
            output.mkdir(parents=True, exist_ok=True)
            output = output / f"scenes.{args.format}"
        elif output.is_dir():
            output = output / f"scenes.{args.format}"
        output.parent.mkdir(parents=True, exist_ok=True)
        try:
            if args.format == "yaml":
                ...
                for s in scenes:
                    ...
                    seen.add(fp)
                    try:
                        render_data.append(_yaml.safe_load(s.read_text(encoding="utf-8")))
                    except Exception:
                        render_data.append({"file": fp, "error": "parse failed"})
                _yaml.safe_dump(render_data, output.open("w", encoding="utf-8"),
                                sort_keys=False, allow_unicode=True)
            else:
                _write_scene_markdown(scenes, project_path, output)
        except Exception as exc:
            _err(f"failed to publish scenes: {exc}")
            return 1
        print(f"Scene artifacts published to {output}")
        return 0
    ...
    if args.command == "identity" and args.identity_command == "validate":
        if not args.identity.exists():
            _err(f"identity file not found: {args.identity}")
            return 1
        try:
            from auteur.identity import StoryIdentity
            ...
            _err(f"invalid story identity {args.identity}: {exc}")
            return 1
        ...
        dd = args.identity.parent / "identity"
        dd.mkdir(parents=True, exist_ok=True)
        ap = dd / "validation_report.json"
        try:
            if serialize_identity_validate(result, dd) is None:
                _err("no data to serialize")
                return 1
        except OSError as exc:
            _err(f"failed to write validation report to {ap}: {exc}")
            return 1
        ...
        if data.has_error:
            ...
            print(f"Validation report written to {ap}", file=sys.stderr)
            return 1
        print(verdict)
        print(f"Validation report written to {ap}")
        return 0
    ...
    if (args.command == "identity" and args.identity_command == "compile") or \
       (args.command == "blueprint" and args.blueprint_command == "seed"):
        ...
        try:
            ...
            _err(f"failed to parse story identity {args.identity}: {exc}")
            return 1
        ...
        try:
            ...
            _err(f"failed to write blueprint to {args.output}: {exc}")
            return 1
    ...
    if args.command == "blueprint" and args.blueprint_command == "publish":
        ...
        if not bp_path.exists():
            _err(f"blueprint not found: {bp_path}")
            return 1
        try:
            bp = StoryBlueprint.from_yaml(bp_path)
        except Exception as exc:
            _err(f"failed to load blueprint: {exc}")
            return 1
        output = args.output
        if output is None:
        ...
        try:
            ...
            _err(f"failed to publish blueprint: {exc}")
            return 1
    ...
    if args.command == "identity" and args.identity_command == "accept-candidate":
        ...
        if result.data.warnings:
            ...
            for w in result.data.warnings:
                ...
                print(f" - {w.message}")
        pdir = args.candidate.parent
        rsp = pdir / "recommendation_set.yaml"
        if rsp.exists():
            try:
    ...
    if args.command == "state":
        if args.state_command == "check":
            ...
            if ol is not None:
                if not ol.exists():
                    _err(f"Outline file not found: {ol}")
                    return 1
                from auteur.structure.outline_audit import load_outline
                try:
                    outline = load_outline(str(ol))
        ...
        if args.state_command == "update":
            ...
            if not result.is_success:
                _err(result.error or "state update failed")
                return result.exit_code
        ...
        if args.state_command == "prepare":
            ...
            if not result.is_success:
                _err(result.error or "state prepare failed")
                return result.exit_code
        ...
        if args.state_command == "confirm":
            ...
            if not result.is_success:
                _err(result.error or "state confirm failed")
                return result.exit_code
    ...
    if args.command == "cartographer":
        if args.cartographer_command == "compile":
            ...
            try:
                ...
                _err(f"failed to build LLM client: {exc}")
                return 1
            ...
            if out:
                ...
                print(format_cartographer_compile_success(str(args.output)))
            return 0
        if args.cartographer_command == "validate":
            ...
            if out:
                ...
                print(format_cartographer_validate_success(str(args.outline)))
            return 0
    # === character ===
...
def _draft_retry(args, *, is_retry: bool) -> int:
    ...
    if not is_retry and result.data is None:
        return result.exit_code
