...
class DependencyRecord(BaseModel):
    ...
    kind: DependencyKind
    source: DependencySource
    projection: Projection = Field(default_factory=lambda: Projection(id="full", fields=[]))
    projected_hash: str | None = Field(default=None, validation_alias=AliasChoices("projected_hash", "content_hash"), serialization_alias="projected_hash")
    full_content_hash: str | None = None
...
class ArtifactStore:
    ...
    def _knowledge_invalid(self, path: Path, metadata: ArtifactMetadata) -> list[str]:
        """Adapt Layer 3 validation into provenance health without owning its rules."""
        if metadata.artifact_type != "scene_realization":
            return []
        try:
            raw = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
            previous_record = next((item for item in metadata.dependencies if item.kind is DependencyKind.STATE_ORDER), None)
            if previous_record and isinstance(raw, dict) and isinstance(raw.get("entry_knowledge"), list):
                previous_raw = yaml.safe_load(self._resolved_path(previous_record.path).read_text(encoding="utf-8")) or {}
                known = {item.get("what") for item in previous_raw.get("exit_knowledge", []) if isinstance(item, dict)}
                return [
                    f"knowledge:impossible_knowledge:{item.get('what')}"
            ...
            reasons = [f"knowledge:{violation.violation_type.value}:{violation.fact_what}" for violation in result.violations]
            previous = next((item for item in metadata.dependencies if item.kind is DependencyKind.STATE_ORDER), None)
            if previous and scene.entry_state:
                predecessor_path = self._resolved_path(previous.path)
                predecessor = SceneLoader().load_scene(str(predecessor_path))
                known = {fact.what for fact in (predecessor.exit_state.knowledge if predecessor.exit_state else [])}
    ...
    def _revision_dir(self, artifact_id: str) -> Path:
        return self.root / "revisions" / artifact_id

    def list_revisions(self, artifact_id: str) -> list[int]:
        directory = self._revision_dir(artifact_id)
        return sorted(int(p.stem) for p in directory.glob("*.yaml") if p.stem.isdigit()) if directory.exists() else []
    ...
    def _write(self, metadata: ArtifactMetadata, *, snapshot: bool = True) -> ArtifactMetadata:
        ...
        payload = metadata.model_dump(mode="json", exclude={"health", "freshness", "summary", "recommended_action"})
        rendered = yaml.safe_dump(payload, sort_keys=False)
        current = self.sidecar_path(metadata.artifact_id)
        temporary = current.with_suffix(".tmp")
        temporary.write_text(rendered, encoding="utf-8")
        temporary.replace(current)
        if snapshot:
            directory = self._revision_dir(metadata.artifact_id)
            directory.mkdir(parents=True, exist_ok=True)
            revision = directory / f"{metadata.revision:06d}.yaml"
            if not revision.exists():
                revision.write_text(rendered, encoding="utf-8")
        return metadata
        ...
        for spec in specs:
            ...
            records.append(
                DependencyRecord(
                    ...
                    kind=spec.kind,
                    source=spec.source,
                    projection=spec.projection or Projection(id="full", fields=spec.fields),
                    fields=spec.fields,
                    revision=dependency.revision if dependency else None,
                    projected_hash=self.content_hash(spec.path, spec.fields) if spec.path.exists() else None,
                    full_content_hash=self.content_hash(spec.path) if spec.path.exists() else None,
                )
            )
        return records
    ...
    def _infer_dependencies(self, path: Path, artifact_type: str) -> list[DependencySpec]:
        ...
        if artifact_type == "blueprint" and identity.exists():
            ...
            chapter_id = self._artifact_id(path)
            if identity.exists():
                specs.append(DependencySpec("story_identity", "story_identity", identity, DependencyKind.SEMANTIC, DependencySource.INFERRED, semantic_fields, identity_projection))
            if blueprint.exists():
                fields = [f"chapters.{chapter_id}"]
                specs.append(DependencySpec("blueprint", "blueprint", blueprint, DependencyKind.STRUCTURAL, DependencySource.INFERRED, fields, Projection(id="blueprint.chapter", version=1, fields=fields)))
        elif artifact_type == "scene_realization":
            try:
                data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
                chapter_id = data.get("chapter_id") or data.get("chapter")
                if blueprint.exists() and data.get("blueprint_fields"):
                    fields = list(data["blueprint_fields"])
                    specs.append(DependencySpec("blueprint", "blueprint", blueprint, DependencyKind.SEMANTIC, DependencySource.INFERRED, fields, Projection(id="blueprint.scene", version=1, fields=fields)))
                elif blueprint.exists() and chapter_id:
                    fields = [f"chapters.{chapter_id}"]
                    specs.append(DependencySpec("blueprint", "blueprint", blueprint, DependencyKind.STRUCTURAL, DependencySource.INFERRED, fields, Projection(id="blueprint.chapter", version=1, fields=fields)))
                if chapter_id:
    ...
    def accept(
        ...
        *,
        ...
        if dependencies is not None:
            ...
            records = self._records(self._infer_dependencies(path, artifact_type))
        acknowledged_reasons = self.status(path, artifact_type).stale_reasons if acknowledge_divergence and previous else []
    ...
    def status(self, path: Path, artifact_type: str) -> ArtifactMetadata:
        ...
        for dependency in metadata.dependencies:
            ...
            if not dep_path.exists():
                ...
                continue
            projection_fields = dependency.projection.fields or dependency.fields
            current_hash = self.content_hash(dep_path, projection_fields)
            current_revision = dep_meta.revision if dep_meta else None
            projection_changed = dependency.projection.version != 1
            if current_hash != dependency.projected_hash or projection_changed:
        ...
        if reasons and metadata.review_state in {ReviewState.NONE, ReviewState.ACKNOWLEDGED_DIVERGENCE}:
            # An acknowledgement applies only to the exact snapshot it reviewed.
            if metadata.review_state is ReviewState.ACKNOWLEDGED_DIVERGENCE:
                metadata.review_state = ReviewState.REVIEW_REQUIRED
            elif metadata.review_state is ReviewState.NONE:
                metadata.review_state = ReviewState.REVIEW_REQUIRED
        return metadata
        ...
        for sidecar in self.root.glob("*.yaml") if self.root.exists() else []:
            ...
            for dependency in metadata.dependencies:
                if dependency.source is not DependencySource.SUGGESTED:
                    dependents.setdefault(dependency.artifact_id, set()).add(metadata.artifact_id)
        result = []
        frontier = [(artifact_id, True)]
        seen = set()
        while frontier:
            source, direct = frontier.pop(0)
            for dependent in sorted(dependents.get(source, set())):
                if dependent in seen:
                    continue
                seen.add(dependent)
                metadata = self._load(dependent)
                result.append({
                    "artifact_id": dependent,
                    "direct": direct,
                    "transitive": not direct,
                    "health": metadata.health if metadata else "unknown",
                    "freshness": metadata.freshness if metadata else "unknown",
                    "review_state": metadata.review_state.value if metadata else "unknown",
                    "reason": f"direct dependency on {source}" if direct else f"transitive impact through {source}",
                })
                frontier.append((dependent, False))
        return result

    def explain(self, path: Path, artifact_type: str) -> list[dict[str, Any]]:
        metadata = self.status(path, artifact_type)
        explanations = []
        for reason in metadata.stale_reasons:
            summary = reason.summary or f"Upstream artifact {reason.dependency_id} changed in a dependency of {metadata.artifact_id}."
            explanations.append({
                **reason.model_dump(mode="json"),
                "summary": summary,
                "status": "stale",
                "acknowledged": metadata.review_state is ReviewState.ACKNOWLEDGED_DIVERGENCE,
                "next_action": "Review and accept the artifact, or acknowledge the divergence with a rationale.",
            })
        for code in metadata.invalid_reasons:
            explanations.append({
                "code": code,
                "summary": f"{metadata.artifact_id} cannot be trusted because {code.replace('_', ' ')}.",
                "status": "invalid",
                "acknowledged": False,
                "next_action": "Restore or correct the source or dependency, then revalidate.",
            })
        return explanations
