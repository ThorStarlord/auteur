...
class BookReconciliationStore:
    ...
    def _validate_acceptance_gate(
        ...
        """Delegate the read-only 20-point Phase C3 gate to its validation seam."""
        validator = BookAcceptanceValidator(
            self,
            error_type=AcceptanceBlockedError,
            comparison_transformation=COMPARISON_TRANSFORMATION,
            supported_comparison_versions=SUPPORTED_COMPARISON_TRANSFORMATION_VERSIONS,
            supported_marker_versions=SUPPORTED_MARKER_CONTRACT_VERSIONS,
            hash_text=_hash,
            book_store_cls=BookExpressionStore,
        )
        return validator.validate(comparison_id)
    ...
    def _validate_completion_gate(
        ...
        """Delegate the read-only 20-point Phase C4 gate to its validation seam."""
        validator = BookCompletionValidator(
            self,
            error_type=CompleteBlockedError,
            supported_marker_versions=SUPPORTED_MARKER_CONTRACT_VERSIONS,
            hash_text=_hash,
            book_store_cls=BookExpressionStore,
        )
        return validator.validate(acceptance_id)
