"""CLI orchestration for the gentle femdom pipeline (sensual dominance, tender surrender, romantic authority)."""
...
class GentlefemdomError(Exception):
    """Base error for gentle femdom CLI operations."""
    pass


class GentlefemdomCommand:
    """Orchestrates the complete gentle femdom pipeline.

    Identical to NetorareCommand but routes to gentle femdom core IDs
    (sensual_dominance, tender_surrender, romantic_authority).
    ...
    def __init__(
        ...
        """Initialize the gentle femdom command.

        Args:
            project_path: Path to project root
            core_id: Core template ID (sensual_dominance, tender_surrender, romantic_authority)
    ...
    def run(self) -> int:
        """Execute the complete gentle femdom pipeline. Returns exit code (0 for success)."""
        try:
            ...
            return 0
        except GentlefemdomError as e:
            self._cleanup()
            print(f"Error: {e}", file=sys.stderr)
            logger.exception(f"Gentle femdom pipeline failed: {e}")
            return 1
        except KeyboardInterrupt:
            self._cleanup()
            print("\nGentle femdom pipeline interrupted", file=sys.stderr)
            return 130
        except Exception as e:
            self._cleanup()
            print(f"Unexpected error: {e}", file=sys.stderr)
            logger.exception(f"Unexpected error in gentle femdom pipeline: {e}")
            return 1
    ...
    def _validate_project_path(self) -> None:
        ...
        if not self.project_path.is_dir():
            raise GentlefemdomError(f"Project path must be a directory: {self.project_path}")
    ...
    def _create_session(self) -> None:
        """Create a new gentle femdom session."""
        if self.session_file.exists():
            raise GentlefemdomError(f"Session already exists at {self.session_file}")
        try:
            self.session_manager = SessionManager.create_session(self.project_path, self.core_id)
        except SessionError as e:
            raise GentlefemdomError(f"Failed to create session: {e}")

    def _start_browser_server(self) -> None:
        """Start the browser server in a subprocess (reuses netorare server)."""
        if not self.session_file.exists():
            raise GentlefemdomError(f"Session file not found: {self.session_file}")
        ...
        try:
            ...
            if self.server_process.poll() is not None:
                ...
                raise GentlefemdomError(f"Server failed to start: {stderr}")
            logger.info(f"Browser server started (PID {self.server_process.pid}) on port {self.port}")
        except Exception as e:
            raise GentlefemdomError(f"Failed to start browser server: {e}")
...
"""
    ...
    def _open_browser(self) -> None:
        """Open browser to the gentle femdom server."""
    ...
    def _poll_for_completion(self) -> None:
        ...
        if not self.session_manager:
            raise GentlefemdomError("Session manager not initialized")
        ...
        raise GentlefemdomError(f"Session did not complete within {self.timeout}s")
    ...
    def _read_and_validate_choices(self) -> dict:
        ...
        if not choices:
            raise GentlefemdomError("No choices found in completed session")
        ...
        try:
            ...
            if not is_valid:
                ...
                raise GentlefemdomError(f"Choices validation failed: {error_msg}")

            logger.info("Choices validated successfully")
            return choices
        except GentlefemdomError:
            raise
        except Exception as e:
            raise GentlefemdomError(f"Validation error: {e}")
    ...
    def _generate_identity(self, choices: dict) -> str:
        ...
        try:
            ...
            raise GentlefemdomError(f"Failed to generate identity: {e}")
        except Exception as e:
            raise GentlefemdomError(f"Unexpected error generating identity: {e}")
    ...
    def _save_identity(self, yaml_content: str) -> None:
        ...
        try:
            ...
            raise GentlefemdomError(f"Failed to save identity file: {e}")
    ...
    def _display_success(self) -> None:
        ...
        print("\n[OK] Gentle femdom pipeline completed successfully!")
        print(f"\nGenerated files:")
...
def handle_gentlefemdom_init(
    ...
    """Handle 'auteur gentlefemdom init' command.
