...
# Fixed ZIP timestamp for deterministic archives. 1980-01-01 00:00:00 is the
# minimum valid value for the ZIP format.
_FIXED_ZIP_DATETIME = (1980, 1, 1, 0, 0, 0)


def _get_auteur_version() -> str:
    """Return the Auteur version from package metadata or fallback."""
    try:
        return importlib.metadata.version("auteur")
...
AUTEUR_VERSION = _get_auteur_version()
...
def _verify_book_integrity(manifest: dict[str, Any], manuscript_bytes: str) -> None:
    """Verify the accepted Book manuscript still matches its recorded hash.

    Raises PublishError with a clear message if the manuscript was modified
    after acceptance.
    """
    recorded_hash = manifest.get("content_hash", "")
    current_hash = _sha256(manuscript_bytes)
    if recorded_hash and current_hash != recorded_hash:
        raise PublishError(
            "Accepted Book manuscript has been modified after acceptance. "
            f"Recorded content hash: {recorded_hash}. "
            f"Current content hash: {current_hash}. "
            "Reconcile the changes or accept the Book again."
        )
    ...
    *,
    ...
    if title_page:
        ...
        parts.append(
            ...
            f"Generated by Auteur v{AUTEUR_VERSION}"
            "</p>"
        )
        parts.append("</div>")
...
def _write_epub_entry(zf: ZipFile, name: str, data: str | bytes) -> None:
    """Write a ZIP entry with deterministic fixed timestamp for EPUB archives."""
    info = ZipInfo(name, date_time=_FIXED_ZIP_DATETIME)
...
def _build_epub_archive(
    ...
    *,
    ...
    with ZipFile(output, "w", ZIP_DEFLATED) as zf:
        # mimetype MUST be the first entry, stored uncompressed (EPUB3 requirement).
        # All entries use fixed ZIP timestamps for deterministic output.
        mt_info = ZipInfo("mimetype", date_time=_FIXED_ZIP_DATETIME)
        zf.writestr(mt_info, "application/epub+zip", compress_type=ZIP_STORED)
        _write_epub_entry(zf, "META-INF/container.xml", _epub_container_xml())
        _write_epub_entry(zf, "OEBPS/stylesheet.css", style)
        _write_epub_entry(zf, "OEBPS/content.opf", _epub_opf_xml(title, author, uid, chapters_html))
        _write_epub_entry(zf, "OEBPS/nav.xhtml", _epub_nav_xhtml(title, chapters_html, uid))
        for ch in chapters_html:
            _write_epub_entry(zf, f"OEBPS/{ch['file']}", ch["content"])

    return {
        "format": "epub",
        "version": 1,
        "output_path": str(output.resolve()),
        "output_hash": _sha256_binary(output.read_bytes()),
...
def _epub_opf_xml(
    ...
    return (
        ...
        f'    <meta property="dcterms:modified">1980-01-01T00:00:00Z</meta>\n'
        f'  </metadata>\n'
        f'  <manifest>\n'
...
class PublishingSnapshot:
    ...
    def __init__(self, project: Path) -> None:
        ...
        # Verify integrity: the manuscript must still match the manifest hash
        _verify_book_integrity(self._manifest, self._markdown)
    ...
    def _immutable_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "chapters": chapters,
            "reproducibility": {
                "auteur_version": AUTEUR_VERSION,
                "markdown_it_version": str(getattr(_md, "VERSION", "0")),
