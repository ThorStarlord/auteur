...
# ---------------------------------------------------------------------------
# PlanningCall — scope-narrowed projection of the blueprint
# ---------------------------------------------------------------------------
...
class ArcDirective(BaseModel):
    ...
    state_summary: str = Field(description="Compact summary of physical/emotional/location state.")
...
class PlanningCall(BaseModel):
    ...
    @classmethod
    def for_chapter(cls, blueprint: StoryBlueprint, chapter_index: int) -> Self:
        if chapter_index < 1 or chapter_index > (blueprint.structure.estimated_chapters or 0):
            raise ValueError(
                f"chapter_index {chapter_index} outside 1..{blueprint.structure.estimated_chapters}."
            )
        ...
        return cls(
            ...
            arc_directives=[_arc_directive(c) for c in blueprint.characters],
...
def _emotional_target_for_act(blueprint: StoryBlueprint, act_index: int) -> str:
...
def _flatten_contract(contract: AuthorAudienceContract) -> list[str]:
    rules = [
