...
key=[l.split("=",1)[1].strip() for l in pathlib.Path(r"C:\Users\Admin\AppData\Local\hermes\.env").read_text().splitlines() if l.startswith("DEEPSEEK_API_KEY=")][0]
from openai import OpenAI
client=OpenAI(base_url=EVALUATOR_BASE_URL, api_key=key)
def call_with_retries(model,messages,max_tokens,temp,top,retries=3):
    for attempt in range(retries+1):
        try:
            t0=time.time()
            r=client.chat.completions.create(model=model,messages=messages,temperature=temp,top_p=top,max_tokens=max_tokens)
            elapsed=int((time.time()-t0)*1000)
            raw=r.choices[0].message.content or ""
            return {"raw_text":raw,"finish_reason":r.choices[0].finish_reason,"usage":{"prompt_tokens":r.usage.prompt_tokens,"completion_tokens":r.usage.completion_tokens,"total_tokens":r.usage.total_tokens,"reasoning_tokens":getattr(getattr(r.usage,'completion_tokens_details',None),'reasoning_tokens',None)},"request_id":r.id,"model":r.model,"elapsed_ms":elapsed},None
...
for pkt in blind_packet:
    ...
    probe=pkt["probe_id"]
    msgs=build_msgs(pkt)
    print(f"Evaluating {opaque} {probe} with chat...")
    res,err=call_with_retries(EVALUATOR_MODEL,msgs,900,0.2,1.0)
    if err or not res:
    ...
    try:
        ...
        print(f"Malformed {opaque}: {raw_j[:500]} err {e}")
...
print(f"New blind eval hash {blind_eval_hash[:16]}")
# update freeze
import json as js
freeze=js.loads((base/"blind-freeze.json").read_text(encoding="utf-8"))
freeze["blind_judgment_hash"]=f"sha256:{blind_eval_hash}"
freeze["freeze_timestamp_utc"]=datetime.datetime.utcnow().isoformat()+"Z"
freeze["evaluator_model_corrected"]="deepseek-chat (was deepseek-reasoner truncated, documented deviation)"
with open(base/"blind-freeze.json","w") as f:
    json.dump(freeze,f,indent=2)
run_lock=js.loads((base/"run-lock.json").read_text(encoding="utf-8"))
manifest=js.loads((base/"generation-manifest.json").read_text(encoding="utf-8"))
...
run_lock["evaluator_provider"]=EVALUATOR_PROVIDER
run_lock["evaluator_model"]=EVALUATOR_MODEL
run_lock["evaluator_version"]=evals[0]["evaluator_returned_model"]
run_lock["protocol_deviation_notes"]="Evaluator switched from deepseek-reasoner to deepseek-chat mid-evaluation due to truncation (reasoning tokens exhausted max_tokens); documented; all 45 judgments ultimately from deepseek-chat with same provider, distinct from generator? Note generator deepseek-chat same model as evaluator after switch - distinct model preference not met, documented."
with open(base/"run-lock.json","w") as f:
    json.dump(run_lock,f,indent=2)
print(f"Done redo tokens in {total_in} out {total_out} est ${actual_est:.4f}")
