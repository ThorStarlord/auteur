...
def resolve(argv: list[str]) -> list[str]:
    ...
    if len(head) > 1 and head[0] == head[-1] and head[0] in "\"'":
        head = head[1:-1]
    # `python` means THE PYTHON RUNNING THIS GATE, never whatever comes first on
    # PATH. Found live on a real factory's first gate run: the gate ran inside a
    # project venv, but `python` resolved to a system interpreter carrying a
    # stale editable install of the product from a long-dead worktree - so the
    # unit rung graded this tree's tests against last month's code. Four phantom
    # failures, every one an assertion about behaviour the stale code predates,
    # and the repo's own import-path test failed exactly as designed. The
    # environment identity of the gate is part of the gate.
...
def main() -> int:
    ...
    if not static_cmd:
        ...
        rc, out = run("static", static_cmd,
                      timeout=int(CONFIG.get("static_timeout_s", 300)))
        if rc != 0:
    ...
    if not unit_cmd:
        ...
        rc, out = run("unit", unit_cmd,
                      timeout=int(CONFIG.get("unit_timeout_s", 300)))
        if rc != 0:
