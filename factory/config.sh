...
# Verified on this machine from git-bash (`claude --version` -> 2.1.101). The
# headless flags (`-p`, `--output-format json`, `--max-budget-usd`) the runner
# passes are Claude-Code-shaped; they get proven, not assumed, on the first
# hand-driven lap. See references/automation.md before trusting any cap flag.
...
# Pinned to the project venv by absolute path, forward slashes, no spaces - the
# dispatcher word-splits this value, so no quotes and no env-var prefixes. The
# harness itself maps `python` to the running interpreter; this pin covers the
# outer invocation. PYTHONIOENCODING=utf-8 rides in the DISPATCH environment
# (hand-lap shell, later the scheduled task) per references/setup.md - Windows
# stdio otherwise eats non-ASCII gate output.
FACTORY_VALIDATE_CMD="${FACTORY_VALIDATE_CMD:-H:/GithubRepositories/auteur-factory/.venv/Scripts/python.exe harness/ci.py}"
...
FACTORY_VALIDATE_QUICK="${FACTORY_VALIDATE_QUICK:-H:/GithubRepositories/auteur-factory/.venv/Scripts/python.exe harness/ci.py --quick}"
...
# AUTEUR: explicit `files` while the machinery is being proven by hand (local
# issues/ dir, no labels, no PRs). Switch to `github` (run init-labels.sh once)
# when the dial moves - the repo is on GitHub and `gh` is authenticated.
FACTORY_BACKEND="${FACTORY_BACKEND:-files}"
