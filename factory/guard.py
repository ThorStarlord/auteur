...
# whatever came back.
#
# Seeded from the auteur interview (R2.1/R2.2) and FACTORY_RULES.md. Every entry
# below was a human decision; removing one is a human commit.
PROTECTED += [
    # CI and repo config: the pipeline definitions must not move under the agent.
    ".github/*",
    # The mission's deferred-roadmap guardrails: release policy and past decisions.
    "docs/adr/*",
    "docs/engineering/release-qualification.md",
    # The PRD the mission was compressed from. The shipped glob only matches
    # *.prd.md; this repo's PRD is docs/PRD.md, so it is listed explicitly.
    "docs/PRD.md",
    # Lockfiles: dependency changes stay human (interview R3, FACTORY_RULES.md 5).
    "pyproject.toml",
    "uv.lock",
]
